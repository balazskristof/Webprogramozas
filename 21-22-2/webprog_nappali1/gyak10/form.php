...
if (isset($_POST['submit'])) { // ha a "submit" létezik a GET/POST paraméterek közt, kaptam valamit
    ...
    if (empty($hibak) && empty($hiba)) {
        ...
        $json[] = [
            ...
            'kedvenc_szam' => intval($_POST['kedvencszam']),
            'palacsinta' => isset($_POST['palacsinta']) ? true : false,
            'palacsintaiz' => $_POST['palacsintaiz']
        ];
        file_put_contents("becenev.json", json_encode($json, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    }
}
?>
<form method="post" novalidate>
    ...
    <!--Milyen palacsintákat szeretsz:
    <input type="radio" name="palacsintaiz" value="lekvar">Lekváros
    <input type="radio" name="palacsintaiz" value="kakaos">Kakaós
    <input type="radio" name="palacsintaiz" value="turos">Túrós<br>-->
